...
def createParser():
     ...
     parser.add_option('--input_gff', dest='input_gff',
                metavar='INPUT', help='Unannotated gff file [OPTIONAL]')
    ...
    for contig_num in  orfids.keys():
       ...
       for orf_num in  orfids[contig_num]:
                ...
                if value < results_dictionary[dbname][orf_id]['value']:
                    ...
                    if output_comp_annot_file1_Str:
                        output_comp_annot_file1_Str += '{0}\t{1}\t{2}\t{3}\t{4}\n'.format('', dbname,\
                               ...
                               str(results_dictionary[dbname][orf_id]['value']*float(weight)))
                    else:
         ...
         if success:  # there was a database hit
            fprintf(output_comp_annot_file1,'%s\n', output_comp_annot_file1_Str)
            fprintf(output_comp_annot_file2,'%s\n', output_comp_annot_file2_Str)
            write_annotation_for_orf(outputgff_file, candidatedbname, dbname_weight, results_dictionary,  contig, candidate_orf_pos,  orf_id, compact_output=compact_output) 
...
def getBlastFileNames(opts) :
    ...
    dbnamePATT = re.compile(r'' + opts.blastdir + '*' + opts.sample_name + '*[.](meta.*)[.]' + opts.algorithm.upper() + 'out.parsed.txt')
